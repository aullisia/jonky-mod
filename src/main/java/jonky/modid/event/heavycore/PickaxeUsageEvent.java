...
public class PickaxeUsageEvent implements PlayerBlockBreakEvents.Before{
    ...
    public boolean beforeBlockBreak(World world, PlayerEntity player, BlockPos pos,
        ...
        if(mainHandItem.getItem() instanceof HeavyPickaxe heavyPickaxe && Boolean.TRUE.equals(mainHandItem.get(ModComponents.TOOL_ABILITY_TOGGLE_COMPONENT)) && player instanceof ServerPlayerEntity serverPlayer) {
            if(HARVESTED_BLOCKS.contains(pos)) {
                return true;
            }

            for(BlockPos position : HeavyPickaxe.getBlocksToBeDestroyed(1, pos, serverPlayer)) {
                if(pos == position || !heavyPickaxe.isCorrectForDrops(mainHandItem, world.getBlockState(position))) {
                    continue;
                }
